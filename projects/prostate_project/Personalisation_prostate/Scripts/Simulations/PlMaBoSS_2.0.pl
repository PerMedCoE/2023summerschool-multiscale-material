...
my $cfgFile=shift;
if (!$cfgFile) 
{printf "Missing cfg file, PlMaBoSS.pl <file.bnd> <file.cfg> <threshold=0.1>\n";
exit;}
my $ProbThreshold=shift;
if (!$ProbThreshold)
{
    $ProbThreshold=0.1;
}
    
my @tempList=split(/\./,$cfgFile);
my $ProjectName=$tempList[0];
if (-e $ProjectName)
{
    my $i;
...
chdir($ProjectName);
system("../../../MaBoSS/Linux/MaBoSS_150n -c ".$cfgFile." -o ".$ProjectName." ".$bndFile); #run MaBoSS
