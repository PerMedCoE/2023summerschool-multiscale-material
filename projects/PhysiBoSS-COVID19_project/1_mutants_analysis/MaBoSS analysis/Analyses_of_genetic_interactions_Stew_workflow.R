...
# import data ----
a1<- read.table("./projectname_epistasis/projectname_norm.xls", header = TRUE, sep="\t", row.names = 1, stringsAsFactors=FALSE)
a2<-a1[,-c(2:5,ncol(a1))]
colnames(a2)[2]<-"HS"
...
b1 <- read.table("./projectname/projectname_probtraj_table.csv", header = TRUE, sep="\t", stringsAsFactors=FALSE)
b1 <- b1[,-ncol(b1)]
...
inputs <- scan("../data/inputs.txt", sep="\n", what = character() )
outputs <- scan("../data/outputs.txt", sep="\n", what = character() )
